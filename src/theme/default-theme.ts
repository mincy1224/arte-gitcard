...
/**
 * Built-in "arte-theme". The warm, brand default.
 * Brand anchors #F2EFE5 / #D77655 / #4A4742 are hard-pinned and must not change.
 *
 * The data palette configures 12 families with a single base color each;
 * deep/lift tones are derived deterministically via OKLCH in the resolver
 * (SPEC §4). `families` order is display-only; renderer assignment uses the
 * resolver's fixed HUE_INTERLEAVE_ORDER.
 */
